---
alwaysApply: true
description: 天机v10.0.1统一规则 - 每次对话自动加载，6规则体系+七大根本原则(含记忆继承与时间维度真值)，涵盖宪法+智能体+质量+操作+开发+常识
---
...
### 原则7: 记忆继承与时间维度真值 (P0·新增·铁律)
- **核心论断**: 任何一项新的设定，必须实施相关记忆，真实继承原体系设定好的内容
- **时间维度真值**: 记忆有时间维度，旧记忆可能过时，最新设定才是真值
- **智能判断**: 需要根据语境智能判断什么是真值（不可机械套用旧记忆）
- **动态更新**: 优先使用 `memory_update` 动态更新已有记忆（非只新增），避免记忆冗余
- **五项可信度判断原则**:
  1. 时间最新优先: created_at/last_accessed最新的记忆优先
  2. 用户当前指令最高优先: 用户最新指令覆盖旧记忆
  3. 详细度优先: 更详细的记忆优先（含完整推导链）
  4. L0用户口误识别: L0层的用户口误不作为真值（如"牟天宗"实为"昊天宗"口误）
  5. access_count辅助判断: 高频访问的记忆通常更重要
- **冲突解决**: 旧记忆与新设定冲突时，以新设定为真值，但必须 `memory_update` 更新旧记忆，禁止保留矛盾记忆
- **继承义务**: 新设定必须真实继承原体系已设定好的内容，不可凭空创造断裂设定
- **违规处理**: 未调用memory_recall的决策 / 未memory_update矛盾记忆 / 凭空创造断裂设定 = P0违规
...
| 文件 | 职责 | 核心内容 | 版本 |
|------|------|---------|------|
| 01-天机宪法v6.0.md | 系统根基 | 身份+四源+路径+README+记忆+依托+双存储+七原则+强制约束+规则淘汰 | v6.0 |
...
**版本**: 3.1.0 | **生效**: 2026-07-05 | **维护**: @tianshu + @yiku
**精简成果**: 7文件→6文件，强壮存留规则+法则
**升级方向**: 智能体调度+MCP规范+工业化生产小说+经验复用
**v3.1升级**: 合并v9.1真值源(§0对话生命周期)+d:\真值源(原则7+七大根本原则)
